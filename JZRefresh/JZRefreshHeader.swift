...
public class JZRefreshHeader: JZRefreshBaseView {
...
struct UnsafePointer {
    static var refreshHeader: UInt32 = 0
    static var refreshFooter: UInt32 = 0
...
public enum JZRefreshState: Int {
    case Stopped
    case Dragging
...
public extension UIGestureRecognizerState {
    func isState(values: [UIGestureRecognizerState]) -> Bool {
        return values.contains({ $0 == self })
    }
}

public extension NSObject {
    func isType(values: [NSObject]) -> Bool {
        return values.contains( {$0 == self})
    }
}
